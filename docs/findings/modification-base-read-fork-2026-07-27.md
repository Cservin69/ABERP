...
| D2c | ~~The same in-serve business-read fork class remains at `serve::read_invoice_currency` (`serve.rs:10496`) and `read_invoice_total_gross_minor` (`:10522`) on the **mark-paid** route, plus the runtime openers at `serve.rs:11034`, `:20968`, `:23208`.~~ **Mark-paid pair CLOSED 2026-08-12** (PR1 of the ADR-0110-rev4 prod-durability series): both take the caller's `state.db.read()` connection, both names are now PINNED, and their lines are removed from the read-fork structural baseline (33→31) and the opener census (81→79 / serve.rs 27→25). The 2026-07-27 reasoning — "all read loudly, so none can mis-file the way D2a could" — was **sound about the read and silent about the close**: these two carried DuckDB's DEFAULT pragmas, with no `disable_checkpoint_on_shutdown`, so each mark-paid call FOLDED and TRUNCATED the live Handle's WAL underneath the writer. That is the 00012 mechanism, and it is a durability defect independent of what the forked reader itself returned. **Still open:** `serve.rs:11034` / `:20968` / `:23208` (`calibration_overview_request`, `resolve_recipient_email`, `handle_quote_pipeline_status` — different tables, not fused with the billing family). | a business-table read-fork sweep of `serve.rs` |
