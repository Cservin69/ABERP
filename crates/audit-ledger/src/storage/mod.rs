...
pub(crate) fn heal_replay_mirror_tail(
    ...
    let mut forensic = Entry {
        ...
        entry_hash: EntryHash::from_bytes([0u8; 32]),
        // Unsigned system/durability event — no login/service session. The
        // session-signing columns are excluded from the canonical `entry_hash`
        // preimage, so leaving them `None` keeps the forensic row's hash exact.
        session_id: None,
        session_pubkey: None,
        event_sig: None,
    };
