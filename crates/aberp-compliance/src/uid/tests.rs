...
fn s358_construct1_builds_and_renders_iri() {
    // IAC "D" (DUNS authority) + EID + part number + serial.
    let c =
        IuidConstruct1::new("D", "0LH12", "BRACKET-7781", "SN-0001").expect("valid construct 1");
    assert_eq!(c.iac(), "D");
...
fn s358_construct1_round_trips_through_serde() {
    let c = IuidConstruct1::new("UN", "CAGE9", "PN-42", "S-99").expect("valid");
...
fn s358_construct2_builds_and_renders_iri() {
    // Construct 2 has no part number — serial is enterprise-wide unique.
    let c = IuidConstruct2::new("D", "0LH12", "SN-0001").expect("valid construct 2");
    assert_eq!(c.iac(), "D");
    assert_eq!(c.eid(), "0LH12");
    assert_eq!(c.serial(), "SN-0001");
...
fn s358_construct2_round_trips_through_serde() {
    let c = IuidConstruct2::new("LH", "12345", "SN-7").expect("valid");
    let json = serde_json::to_string(&c).expect("serialize");
    let back: IuidConstruct2 = serde_json::from_str(&json).expect("deserialize");
    assert_eq!(c, back);
    assert_eq!(back.to_iri(), "LH12345SN-7");
}

#[test]
fn s358_iuid_enum_dispatches_iri_and_construct_code() {
    let c1 = Iuid::Construct1(IuidConstruct1::new("D", "0LH12", "PN-1", "S-1").expect("c1"));
    let c2 = Iuid::Construct2(IuidConstruct2::new("D", "0LH12", "S-1").expect("c2"));
    assert_eq!(c1.to_iri(), "D0LH12PN-1S-1");
    assert_eq!(c2.to_iri(), "D0LH12S-1");
...
fn s358_iuid_enum_round_trips_through_serde() {
    let original =
        Iuid::Construct1(IuidConstruct1::new("UN", "CAGE9", "PN-42", "S-99").expect("c1"));
...
fn s358_construct_new_rejects_invalid_components() {
    // Bad IAC propagates from validate_iac.
    assert_eq!(
        IuidConstruct1::new("abc", "0LH12", "PN-1", "S-1"),
        Err(UidError::TooLong {
            field: "iac",
            len: 3,
            max: MAX_IAC_LEN,
    ...
    assert_eq!(
        IuidConstruct2::new("D", "0LH12", ""),
        Err(UidError::Empty { field: "serial" })
    );
    // Whitespace in the enterprise id is an invalid char.
    assert_eq!(
        IuidConstruct1::new("D", "0L H12", "PN-1", "S-1"),
        Err(UidError::InvalidChar {
            field: "eid",
            ch: ' ',
        })
    );
    // A field over the ceiling is rejected with its length.
    let too_long = "A".repeat(MAX_UID_FIELD_LEN + 1);
    assert_eq!(
        IuidConstruct1::new("D", "0LH12", &too_long, "S-1"),
        Err(UidError::TooLong {
