...
//!
//! The shared in-process [`crate::Handle`] (H3) guarantees ONE DuckDB instance
...
//! `aberp::submission_lock`: one `fs2` advisory file lock (flock on
...
//! window).
//!
//! # ADR-0110 D9 — WHY THIS LIVES IN `aberp-db` AND NOT IN `apps/aberp`
//!
//! It shipped in `apps/aberp/src/db_writer_lock.rs` while every caller was an
//! `aberp` subcommand. It is not: `crates/aberp-inventory`'s
//! `rebuild-stock-cache` recovery binary is a separate package that ADR-0061
//! tells operators to run, and it cannot depend on `apps/aberp`. The choice was
//! therefore between a SECOND copy of [`lock_path_for`] and one shared module —
//! and a second copy is the worse hazard by a distance: two derivations that
//! drift by one character produce two lock files, and two lock files are no lock
//! at all, silently. So the module moved down to the crate that already owns
//! "one writer per tenant DB", and `apps/aberp` re-exports it — every existing
//! `crate::db_writer_lock::…` call site is untouched.
//!
...
/// Typed failure of the whole-DB writer lock.
///
/// `aberp-db` is a library crate (ADR-0021 Part A: no `anyhow` below the app
/// boundary), so the module that moved down here trades its `anyhow::Error` for
/// this. The `Display` bodies are carried over VERBATIM — the CLI refusal
/// message an operator sees, and the `"single-writer"` substring the F-E
/// refusal tests match on, are both load-bearing.
#[derive(Debug, thiserror::Error)]
pub enum DbWriterLockError {
    /// The DB path has no parent directory to place the lock file in.
    #[error("tenant db path `{0}` has no parent dir for the whole-DB writer lock")]
    NoParentDir(PathBuf),

    /// The lock file itself could not be opened (fs error) — loud-fail rather
    /// than silently skipping the lock.
    #[error("open whole-DB writer lock file {path}: {source}")]
    Open {
        path: PathBuf,
        source: std::io::Error,
    },

    /// `flock` failed for a reason other than "already held".
    #[error("acquire exclusive whole-DB writer lock {path}: {source}")]
    Acquire {
        path: PathBuf,
        source: std::io::Error,
    },

    /// Another writer holds the tenant lock — the operator-facing refusal.
    #[error(
    ...
    Contended { who: String, tenant: String },
}
...
/// posture as `aberp::submission_lock`).
...
fn lock_path_for(db_path: &Path, tenant: &str) -> Result<PathBuf, DbWriterLockError> {
    let parent = db_path
        .parent()
        .filter(|p| !p.as_os_str().is_empty())
        .ok_or_else(|| DbWriterLockError::NoParentDir(db_path.to_path_buf()))?;
...
pub fn try_acquire(
    db_path: &Path,
    tenant: &str,
) -> Result<Option<DbWriterLockGuard>, DbWriterLockError> {
    ...
    let file = OpenOptions::new()
        ...
        .write(true)
        .open(&path)
        .map_err(|source| DbWriterLockError::Open {
            path: path.clone(),
            source,
        })?;
    match file.try_lock_exclusive() {
        ...
        Err(source) => Err(DbWriterLockError::Acquire { path, source }),
    }
}
...
pub fn acquire_or_refuse(
    ...
    tenant: &str,
    who: &str,
) -> Result<DbWriterLockGuard, DbWriterLockError> {
    match try_acquire(db_path, tenant)? {
        Some(guard) => Ok(guard),
        None => Err(DbWriterLockError::Contended {
            who: who.to_string(),
