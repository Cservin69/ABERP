...
| --- | --- | --- | --- |
| `SupplierDpasPrioritySet` | `supplier.dpas_priority_set` | a DPAS priority rating is assigned to a supplier | `{partner_id, dpas_rating, set_by_operator_id, set_at_ms}` |
| `SupplierExportScreened` | `supplier.export_screened` | a supplier is screened against the denied-party lists | `{partner_id, screening_result, screening_source, screened_at_ms, screened_by_operator_id, hit_details?}` |
...
| column | type | holds |
| --- | --- | --- |
| `dpas_rating` | `VARCHAR` nullable | the `DpasRating::as_str` form (`NONE` / `DO-C1` / `DX-C1`) |
...
**Validation lives at the (future) write boundary, never as a DB CHECK** (per `[[no-sql-specific]]`): the firing site validates an inbound `dpas_rating` / `export_screening_status` through `DpasRating::from_storage_str` / `ExportScreeningStatus::from_storage_str` before it reaches the column, so the column only ever holds a well-formed token. `eccn` is deliberately unvalidated — an ECCN is a structured but open vocabulary (`7A994`, `EAR99`, …) the classification service determines, not a closed enum.
