...
describe("notification-prefs", () => {
  ...
  it("round-trips all toggles", () => {
    const s = memStorage();
    saveNotificationPrefs(
      { nativeEnabled: true, soundEnabled: true, adapterSoundEnabled: true },
      s,
    );
    expect(loadNotificationPrefs(s)).toEqual({
      nativeEnabled: true,
      soundEnabled: true,
      adapterSoundEnabled: true,
    });
  });
  ...
  it("never round-trips unknown keys back to storage", () => {
    const s = memStorage();
    saveNotificationPrefs(
      { nativeEnabled: true, soundEnabled: false, junk: "x" } as never,
      s,
    );
