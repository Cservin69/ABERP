...
describe("the past-deadline HYGIENE facet keeps excluding undated rows", () => {
  ...
  // so imputing 90+ is defensible and disclosed. `payable_past_deadline_count`
  // is a LATENESS ASSERTION, and nothing supports one for an invoice with
  // no deadline — so the backend counter skips undated rows
  // (`reports::aggregate_ap`) and this list, which is that tile's
  // drill-down, must skip them too. Dropping this early-out to "match"
  // the aging facet would make the list over-count the tile instead.
